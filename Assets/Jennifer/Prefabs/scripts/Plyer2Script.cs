...
public class Plyer2Script : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetKeyUp(KeyCode.RightShift) && thrown == true)
        {
            item = null;
            check = false;
            thrown = false;
    ...
    private void OnTriggerStay(Collider other)
    {
        ...
        { // pick up - 1
            if (other.tag == "pickUp" && picked == false && Input.GetKeyDown(KeyCode.RightShift))
            {
            ...
            // after pickup - 2
            if (Input.GetKeyUp(KeyCode.RightShift) && check == false && picked == true)
            {
                check = true;
                picked = false;
            ...
            if (picked)
            {// Throw - 3
                if (Input.GetKeyDown(KeyCode.RightShift) && check == true)
                {
                    thrown = true;
                    check = false;
                    Throwing();
